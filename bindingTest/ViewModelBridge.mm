...
static ViewModelBridge* gInstance = nil;

namespace {
	
class CViewModelListener : public IViewModelListener {
	
public: CViewModelListener(ViewModelBridge* iMainViewModel) :
...
private: ViewModelBridge* fMainViewModel;
};
...
void CreateMainViewModelInstance() {
	...
	gInstance = [[ViewModelBridge alloc] init];
}
...
@implementation ViewModelBridge {
...
- (void)removeArrayValueAtIndexes:(NSIndexSet*)indexes
{
	[indexes enumerateRangesWithOptions:NSEnumerationReverse usingBlock:^(NSRange range, BOOL * _Nonnull stop) {
		NSAssert( range.length == 1, @"Only support removal of one element at a time" );
